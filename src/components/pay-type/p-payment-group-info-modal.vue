<template>
    <u-modal :visible="visible" :title="type ? '新增支付类型分组' : '编辑支付类型分组'" @before-close="submit" @close="closeModal" class="s-store-modal">
        <el-form ref="form" :model="form" label-width="110px">
            <el-form-item v-if="!type" label="支付类型分组ID">
...
export default {
    ...
    created() {
        this.$bus.$on('open-payment-group-info-modal', (groupDetail, isAdd) => {
            this.type = isAdd
            this.form = this.type ? { status: true } : { ...groupDetail }
        })
    },
    destroyed() {
        this.$bus.$off('open-payment-group-info-modal')
    },
    methods: {
        submit(e) {
            e.preventDefault()
