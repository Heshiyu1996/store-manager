<template>
    <u-layout class="pay-type-group-list" direction="v">
        <div class="top-wrapper">
            <u-layout>
                ...
                <u-input v-model.trim="searchParams.name" maxLength="100" placeholder="请输入支付类型分组" searchIcon @key-press-enter="_getList" />
            </u-layout>
        ...
        <u-layout class="content-wrapper" direction="v">
            <u-table ref="operationTable" :list="list" auto is-list>
                <template slot-scope="{ row }">
                    ...
                    <u-table-column width="24vw" label="支付类型分组" ellipse>{{ row.name || '-' }}</u-table-column>
                    <u-table-column width="24vw" label="所属门店" ellipse> {{ row.storeName }} </u-table-column>
                    <u-table-column width="24vw" label="使用状态" ellipse>
        ...
        <PPaymentGroupInfoModal :visible="isOpenPaymentGroupInfoModal" @close="closePaymentGroupInfoModal" />
    </u-layout>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            isOpenPaymentGroupInfoModal: false,
    ...
    methods: {
        addPatch() {
            this.$bus.$emit('open-payment-group-info-modal', {}, MODIFY_MODAL_TYPE.ADD)
            this.isOpenPaymentGroupInfoModal = true
        },
        updateRows(otype) {
            ...
            switch (otype) {
                ...
                case OPERATION_TYPE.CLOSE:
                    ...
                    updatePaymentGroupStatus({ ids: params, status: isOpen }).then(() => {
        ...
        editRow(row) {
            ...
            this.$bus.$emit('open-payment-group-info-modal', row, MODIFY_MODAL_TYPE.EDIT)
            this.isOpenPaymentGroupInfoModal = true
        },
        deleteRow(...ids) {
        ...
        _getList(isNew) {
            ...
            getPaymentGroupList(this.searchParams).then(data => {
                this.list = data.list || []
