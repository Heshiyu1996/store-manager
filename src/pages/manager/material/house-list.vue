<template>
    <u-layout class="house-list" direction="v">
        ...
        <u-layout class="content-wrapper" direction="v">
            <u-table ref="operationTable" :list="list" auto is-list>
                <template slot-scope="{ row }">
                    ...
                    <u-table-column width="14vw" label="仓库名称" ellipse>{{ row.name || '-' }}</u-table-column>
                    <u-table-column width="14vw" label="地址" ellipse> {{ row.address }} </u-table-column>
                    <u-table-column width="14vw" label="所属门店" ellipse> {{ row.storeName }} </u-table-column>
                    <u-table-column width="14vw" label="备注" ellipse> {{ row.remark }} </u-table-column>
...
import { getMaterialHouseList, deleteMaterialHouse, updateMaterialHouseStatus } from '@/server/api'
...
export default {
    ...
    methods: {
        ...
        _getList(isNew) {
            ...
            getMaterialHouseList(this.searchParams).then(data => {
                this.list = data.list || []
